...
public class OnlineCoursesAnalyzer {
    ...
    public OnlineCoursesAnalyzer(String datasetPath) throws IOException {
        courseList = Files.lines(Paths.get(datasetPath))
                ...
                .map(a->new course(a[0],a[1],a[2],a[3], a[4],a[5],
                        ...
                        Double.parseDouble(a[12]),Double.parseDouble(a[13]),Double.parseDouble(a[14]),
                        Double.parseDouble(a[15]),Double.parseDouble(a[16]),Double.parseDouble(a[17]),
                        Double.parseDouble(a[18]),Double.parseDouble(a[19]),Double.parseDouble(a[20]),
                        Double.parseDouble(a[21]),Double.parseDouble(a[22]))).toList();
    }
    ...
    public Map<String,Integer> getPtcpCountByInst(){
        ...
        /*
        List<Map.Entry<String, Integer>> list = new ArrayList<Map.Entry<String, Integer>>(sum.entrySet());
        ...
        courseList.forEach(o->{
            o.getCourseSubjectDivide().forEach(l->{
                if(result.containsKey(o.getInstitution()+"-"+l)){
                    result.put(o.getInstitution()+"-"+l,result.get(o.getInstitution()+"-"+l)+o.getParticipants());
                }
                else{
    ...
    */
    ...
    public Map<String,Integer> getPtcpCountByInstAndSubject(){
        ...
        courseList.forEach(o->{
            if(result.containsKey(o.getInstitution()+"-"+o.getCourseSubjectPre())){
                result.put(o.getInstitution()+"-"+o.getCourseSubjectPre(),result.get(o.getInstitution()+"-"+o.getCourseSubjectPre())+o.getParticipants());
            }
            else{
    ...
    public List<String> getCourses(int topK, String by){
        ...
        if(by.equals("hours")){
            courseList.stream().collect(Collectors
                            .groupingBy(course::getCourseTitlePre,Collectors.maxBy(Comparator.comparingDouble(course::getCourseHours))))
                    .forEach((o1,o2)-> result.put(o1,o2.get().getCourseHours()));
            //result.keySet().forEach(System.out::println);
            //result.entrySet().stream().sorted(Map.Entry.<String,Double>comparingByValue().reversed()).limit(topK).forEach(o->ans.add(relate.get(o.getKey())));
            result.entrySet().stream().sorted(Map.Entry.comparingByKey())
    ...
    public List<String> searchCourses(String courseSubject, double percentAudited, double totalCourseHours){
        List<String> ans = new ArrayList<>();
        courseList.stream().filter(o->o.getAuditedPercent()>=percentAudited).filter(o->o.getCourseHours()<=totalCourseHours)
    ...
    public List<String> searchCourses(String courseSubject, double percentAudited, double totalCourseHours){
        ...
        courseList.stream().filter(o->o.getAuditedPercent()>=percentAudited).filter(o->o.getCourseHours()<=totalCourseHours)
                .forEach(o->{
                    if(o.getCourseSubjectPre().toLowerCase().contains(courseSubject.toLowerCase())){
                        ans.add(o.getCourseTitlePre());
                    }
                });
        return ans.stream().distinct().sorted(String::compareTo).toList();
    ...
    public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher){
        ...
        Map<String,List<course>> courseSet =courseList.stream().collect(Collectors.groupingBy(course::getCourseNumber));
        ...
        Map<String,Double> medianAge = courseList.stream()
                .collect(Collectors.groupingBy(course::getCourseNumber,Collectors.averagingDouble(course::getMedianAge)));
        Map<String,Double> medianGender = courseList.stream()
                .collect(Collectors.groupingBy(course::getCourseNumber,Collectors.averagingDouble(course::getMalePercent)));
        Map<String,Double> medianDegree = courseList.stream()
                .collect(Collectors.groupingBy(course::getCourseNumber,Collectors.averagingDouble(course::getDegreePercent)));
        ...
        /*
        similarity.entrySet().stream().sorted(Map.Entry.comparingByValue()).forEachOrdered(o->choose.add(o.getKey()));
    ...
    public static class course {
        ...
        private String courseTitlePre;
        private String instructors;
        private String instructorPre;
        private List<String> instructorDivide;
        private boolean oneInstructor;
        private String courseSubject;
        private String courseSubjectPre;
        private List<String> courseSubjectDivide;
        private int Year;
        ...
        private double certifiedPercentHalf;
        private double playedVideoPercent;
        private double postedPercent;
        private double higherThanZeroPercent;
        ...
        public course(String institution, String courseNumber, String launchDate, String courseTitle, String instructors, String courseSubject, int year, int honorCode, int participants, int audited, int certified, double auditedPercent, double certifiedPercent, double certifiedPercentHalf, double playedVideoPercent, double postedPercent, double higherThanZeroPercent, double courseHours, double medianHours, double medianAge, double malePercent, double femalePercent, double degreePercent) {
